...
export async function findAccountsAtRisk(
  ...
  await Promise.all(
    accounts.map((account: anchor.ProgramAccount) => {
      ...
      console.log(
        `[ACCOUNT_AT_RISK] [ACCOUNT]: ${account.publicKey.toString()} [BALANCE]: ${marginAccountState.balance
        } [INITIAL] ${marginAccountState.initialMargin} [MAINTENANCE]: ${marginAccountState.maintenanceMargin
        } [UNREALIZED PNL] ${marginAccountState.unrealizedPnl
        } [AVAILABLE BALANCE INITIAL] ${marginAccountState.availableBalanceInitial
        } [AVAILABLE BALANCE MAINTENANCE] ${marginAccountState.availableBalanceMaintenance
        }`
      );
...
export async function findLiquidatableAccounts(
  ...
  await Promise.all(
    accounts.map((account: anchor.ProgramAccount) => {
      ...
      console.log(
        `[LIQUIDATABLE ACCOUNT] [ACCOUNT] ${account.publicKey.toString()} [BALANCE] ${marginAccountState.balance
        } [AVAILABLE BALANCE MAINTENANCE] ${marginAccountState.availableBalanceMaintenance
        }`
      );
